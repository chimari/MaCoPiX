...
▼動作環境
   [for Microsoft Windows (バイナリ配布)]
     ・Windows7以降 (10, 64bit 推奨)
     ・TAR64(or 32).DLL(GPL)を同梱
        ※参照「統合アーカイバプロジェクト」
...
▼本体のインストール～起動
  ...
  1. MaCoPiX-win  本体のインストーラをダウンロードして起動
  2. MaCoPiX本体 (macopix.exe)  を起動する。
  3. "MaCoPiX起動中"のメッセージウィンドウから
         "公式マスコットのダウンロード"
     を選択してとりあえずなにかひとつダウンロードしてみる。
     (ネットワーク接続が必要になります)
  4. マスコットが自動的にインストールされる
      (C:\User\(user name)\AppData\Roaming\MaCoPiX\UserData\)。

  また、すでにお気に入りの手持ちマスコットを他で入手している場合は
  上記(3)の段階でメニューからそのファイルを指定してインストールする
  こともできます。

▼マスコットの追加インストール
  a. 公式マスコットの場合は
     マスコットを右クリックしてくるメニューから
     "インストール"→"公式マスコットのダウンロード"
     を選択すると、リストから好きなものを選択してダウンロードおよび
     インストールすることができます。

  b. インストールしたいMaCoPiX用マスコットアーカイブ(.tar.gz)を
     持っている場合は
     マスコットを右クリックすると出てくるメニューから
     "インストール"→"ランチャメニュー"
     を選択。
     ダウンロードしたマスコットアーカイブファイル(.tar.gz)を指定します。

　マスコットは自動的に
      C:\User\(user name)\AppData\Roaming\MaCoPiX\UserData\
  (環境変数 %APPDATA%を参照)下にインストールされます。
...
▼よくある質問
  ...
  A. インストール済みのマスコットについてはコマンドラインオプションで
     ...
     .mcpxファイルはUserDataディレクトリの下にインストールされています。
  ...
  A. ひとつの設定ファイルにはひとつのメイルアカウントしか登録できません。
     ...
     標準の設定ファイルは
      C:\User\(user name)\AppData\Roaming\MaCoPiX\UserData\macopix.ini
     ファイルです。(環境変数 APPDATAを参照しています)
  ...
  Q. タスクトレイアイコンがでない.
  A. Gtk+3版ではタスクトレイアイコンは廃止されました。



▼Windows版をソースからコンパイル
   msys2 に Gtk+3 もしくは Gtk+2, OpenSSLの開発環境をインストール、
   そしてTAR64(or 32).DLLを用意(インストール)しておきます。
   またUNLHA32.DLL(32 bitでコンパイルする場合のみ)を使うようにコンパイル
   することもできます。その場合は以下にしたがってそちらも用意します。
   - msys2上の個人ディレクトリに sourceを展開。
   ...
   - make用のスクリプトがあるのでsourceアーカイブ中で
      ...
      % ./makewin32.sh 
     を実行すればよい。
   - $HOME/dist64/ もしくは $HOME/dist32/ の下にmakeされる。
